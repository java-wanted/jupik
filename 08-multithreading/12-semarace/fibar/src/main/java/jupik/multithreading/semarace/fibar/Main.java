package jupik.multithreading.semarace.fibar;
...
public class Main
{
    static class Vfs
    {
        static final int N_RES = 3;
        static final int N_PAR = 2;
        static Semaphore sem = new Semaphore(N_RES);
        static CyclicBarrier bar = new CyclicBarrier(N_PAR);
        static CyclicBarrier ebar = new CyclicBarrier(N_PAR);

        protected static void baccess(int id) throws InterruptedException
        {
            ...
            try
            {
                ...
                System.out.printf("%s: #%d starts vfs bar access\n", name, id);
                Thread.sleep(500);

                ebar.await();
                System.out.printf("%s: #%d completed vfs bar access\n", name, id);
            }
